...
# Undefined
Undefined - также, как и `null` является признаком отсутствия значения, но более глубоким. Это значение переменных, которые небыли инициализированы или не существуют. Также является возвращаемым значением для [функций](Функции), которые явно не возвращают значение и значением параметров [функций](Функции) параметры для которых не передавались. Если использовать операцию `typeof` на `undefined`, то нам вернется `undefined`.
